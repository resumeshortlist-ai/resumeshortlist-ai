...
export default async function SuccessPage({
  ...
  // If Stripe isn't configured, show a helpful message (instead of crashing)
  if (!stripe) {
    return (
      <main style={styles.main}>
        ...
        <div style={styles.actions}>
          <Link href="/app" style={styles.primaryBtn}>Back to Upload</Link>
          <Link href="/" style={styles.secondaryBtn}>Home</Link>
        </div>
      </main>
    );
  }

  // If the user lands here without the session id
  if (!sessionId) {
    return (
      <main style={styles.main}>
        <h1 style={styles.h1}>Missing session</h1>
        <p style={styles.p}>
          We couldn’t find a Stripe session ID. Please return and try checkout again.
        </p>
        <div style={styles.actions}>
          <Link href="/app" style={styles.primaryBtn}>Back to Upload</Link>
          <Link href="/" style={styles.secondaryBtn}>Home</Link>
  ...
  return (
    <main style={styles.main}>
      <h1 style={styles.h1}>{paid ? "Payment successful ✅" : "Payment not confirmed ⚠️"}</h1>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  card: { border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, marginTop: 20 },
